...
class Simple_DeepBeliefNet():    

    ''' 
    Design a simpler network than in the previous task, 
    with one hidden layer removed, i.e. 784, 500+10, 2000. 
    Pretrain it 􏰃rst using the CD algorithm, as described in 
    section 3.2 (see also section 4.2), and then 􏰃ne-tune it 
    with the wake-sleep learning. Make a thorough comparative 
    analysis of the generalization performance (classi􏰃cation 
    accuracy reported on the test set) obtained with the pretrained 
    vs 􏰃ne-tuned DBNs.
    ...
    def recognize(self,true_img,true_lbl, k=None):
        ...
        #ToDo: Calculate h with (get_h_given_v_dir) with rbm vis-pen
        p_pen, pen = self.rbm_stack['vis--pen'].get_h_given_v_dir(vis) 
    ...
    def generate(self,true_lbl,name):
        ...
        for i in tqdm.tqdm(range(self.n_gibbs_gener)):
            ...
            p_vis, vis = self.rbm_stack["vis--pen"].get_v_given_h_dir(pen_lbl[:,self.n_labels:]) 
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations, train_top_layer=True, img_dir=''):
        ...
        # Load First Layer
        try :
            self.loadfromfile_rbm(loc="trained_rbm_simple",name="vis--pen")
            self.rbm_stack["vis--pen"].untwine_weights() 
        except IOError :
            print ("training vis--pen")
            # Gibbs sampling k=1 for vis to pen
            self.rbm_stack["vis--pen"].cd1(visible_trainset=vis_trainset, n_iterations=n_iterations, img_dir=None)
            self.savetofile_rbm(loc="trained_rbm_simple",name="vis--pen")
            self.rbm_stack["vis--pen"].untwine_weights()

        # Load top layer
        if train_top_layer:
            try: 
                self.loadfromfile_rbm(loc="trained_rbm_simple",name="pen+lbl--top")      
            except IOError :
                print ("training pen+lbl--top")
                pen_trainset = self.rbm_stack["vis--pen"].get_h_given_v_dir(vis_trainset) [0]
                ...
                self.savetofile_rbm(loc="trained_rbm_simple",name="pen+lbl--top")            
    ...
    def train_wakesleep_finetune(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try :
            ...
            self.loadfromfile_dbn(loc="trained_dbn_simple",name="vis--pen")
            self.loadfromfile_rbm(loc="trained_dbn_simple",name="pen+lbl--top")
            ...
            for it in tqdm.tqdm(range(n_iterations)): 
                ...
                p_gen_vis, gen_vis = self.rbm_stack["vis--pen"].get_v_given_h_dir(pen_lbl_k[:,self.n_labels:])  
                

                ########################## Update generative parameters (page 28)  ####################
                p_preds_vis, _ = self.rbm_stack["vis--pen"].get_v_given_h_dir(pen) 
                self.rbm_stack["vis--pen"].update_generate_params(pen, vis, p_preds_vis) 
                ...
                p_preds_pen, _ = self.rbm_stack["vis--pen"].get_h_given_v_dir(gen_vis) 
                self.rbm_stack["vis--pen"].update_recognize_params(gen_vis, pen_lbl_k[:,self.n_labels:],  p_preds_pen) 
                        
            self.savetofile_dbn(loc="trained_dbn_simple",name="vis--pen")
            self.savetofile_rbm(loc="trained_dbn_simple",name="pen+lbl--top")            
